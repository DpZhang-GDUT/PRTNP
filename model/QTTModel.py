...
class QTTModel(BaseTNModel):
    def __init__(self, target, init_reso, max_rank=256, dtype='float32', loss_fn_str="L2", use_TTNF_sampling=False, wavelet_recon_min_targets=0, payload=0, payload_position='first_core', canonization="first", activation="None", compression_alg="compress_all", regularization_type="TV", dimensions =2, regularization_weight = 0.0, noisy_target = None, device = 'cpu',  masked_avg_pooling = False, sigma_init=0, diff_scale=1, ranks_for_upsampling = None):
        """
        ...
        - wavelet_recon_min_targets: minimum number of targets for wavelet reconstruction.
        ...
        super().__init__(target, init_reso, max_rank, dtype, loss_fn_str, use_TTNF_sampling, payload, payload_position, canonization, activation, compression_alg, regularization_type, dimensions, noisy_target, device, masked_avg_pooling, sigma_init, ranks_for_upsampling)
        self.model = "QTT"
        self.canonization = canonization
        self.compression_alg = compression_alg
        self.regularization_type = regularization_type
        self.activation = activation
        self.wavelet_recon_min_targets = wavelet_recon_min_targets
    ...
    def create_rank_upsampling_mask(self):
        ...
        for i,c in enumerate(self.tn.tensors):
            ...
            masked_components.append(mask.to(self.device))
    ...
    def upsample(self, iteration):
        ...
        self.downsampled_target_temp = self.downsampled_target.clone()  # keep a copy of the downsampled target for the next iteration
    ...
    def get_reconstructed_values(self, x):
        ...
        if self.use_TTNF_sampling:
            ...
            self.current_image = image.clone()  # keep a copy of the image for the next iteration
    ...
    def rank_from_discrepancy(self, X, delta2):
        ...
        S = torch.linalg.svdvals(M)                    # [min(H,W*C)]
        S2 = S**2
        # tail(r) = sum(S^2) - cumsum(S^2)[r-1]
        tail = torch.flip(torch.cumsum(torch.flip(S2, dims=[0]), dim=0), dims=[0])
        r = int((tail < delta2).sum().item()) + 1
        r = max(1, min(r, S.numel()))                  # 安全
        return r
    ...
    def nuclear_norm_regularization(self, tn, weight=1e-4):
        ...
        for tensor in tn.tensors:
            ...
            elif len(data.shape) == 2:  # (r1, n)
                nuclear_norm = torch.norm(data, p='nuc')
                total_nuclear_norm += nuclear_norm
        
        return weight * total_nuclear_norm

    def forward1(self, x, x_norm = None):
        """
        ...
        values_target = self.get_values_for_coords(self.downsampled_target.to(self.device), x)

        loss =  self.loss_fn(values_recon, values_target)

        return loss, reg_term
    
    def forward(self, x, x_norm, recon_targets, ite, iterations):
        """
        The forward pass for the model.
        ...
        if len(recon_targets) > 10:  #and len(recon_targets) < 2
            ...
            sigma = 10  * std_d  #* len(recon_targets)
            noise = torch.randn_like(self.downsampled_target) * sigma
            noise_downsampled_target = self.downsampled_target + noise

            values_target = self.get_values_for_coords(noise_downsampled_target, x)
        else:
        ...
        loss +=  self.l2_loss(values_recon, values_target*self.diff_scale)/(self.diff_scale**2)
        #loss +=  self.l2_loss(values_recon, values_target)
    ...
    def print_model_info(self):
        ...
        print(f"张量网络维度: {self.dimensions}D")
        print(f"最大秩: {self.max_rank}")
        print(f"当前分辨率: {self.current_reso}")
        
        # 张量网络结构
        print(f"\n张量网络结构:")
        print(f"核心张量数量: {len(self.tn.tensors)}")
        for i, tensor in enumerate(self.tn.tensors):
            shape = tensor.data.shape
            params = tensor.data.numel()
            print(f"  核心 {i}: 形状 {tuple(shape)}, 参数量 {params:,}")
        
        # 参数量统计
        print(f"\n参数量统计:")
        print(f"可训练参数总数: {self.num_trainable_params:,}")
        print(f"压缩后参数量: {self.num_compressed_params:,}")
        print(f"未压缩参数量: {self.num_uncompressed_params:,}")
        print(f"压缩比: {self.compression_factor:.2f}x")
        
        # 内存占用
        print(f"\n内存占用统计:")
        print(f"压缩后内存: {self.sz_compressed_gb:.4f} GB")
        print(f"未压缩内存: {self.sz_uncompressed_gb:.4f} GB")
        print(f"内存节省: {(1 - self.sz_compressed_gb/self.sz_uncompressed_gb)*100:.2f}%")
        
        # 计算复杂度
        print(f"\n计算复杂度:")
        print(f"FLOPS: {self.flops:,}")
        print(f"最大中间张量: {self.largest_intermediate:,}")
