...
def get_model_args(args, target, noisy_target, device_type):
    ...
    model_args = {
        ...
        "ranks_for_upsampling": args.ranks_for_upsampling,
    }
    ...
    if args.model == "QTT":
        model_args.update({
        'sigma_init': args.sigma_init,
        'wavelet_recon_min_targets': getattr(args, 'wavelet_recon_min_targets', 0),
        })
...
def visualize_and_save_best_recon(best_recon, target, args, save_path=None):
    """可视化重构结果和目标图像并保存
    ...
    if save_path is None:
        save_dir = f'reconstructions/{args.data}_{args.end_reso}/{args.model}'
        os.makedirs(save_dir, exist_ok=True)
        save_path = f'{save_dir}/recon_{time.strftime("%Y%m%d_%H%M%S")}.png'
    
    # 确保是CPU上的numpy数组
    if torch.is_tensor(best_recon):
        best_recon_np = best_recon.detach().cpu().numpy()
    else:
        best_recon_np = best_recon
    ...
    # 保存重构张量
    recon_tensor_path = f"111_reconstruction.pt"
    torch.save(best_recon, recon_tensor_path)
    print(f"重构张量已保存至: {recon_tensor_path}")
    
    # 保存目标张量
    target_tensor_path = f"111_target.pt"
    torch.save(target, target_tensor_path)
    print(f"目标张量已保存至: {target_tensor_path}")
    ...
    if args.dimensions == 2:
        # 目标图像
        im1 = axes[0].imshow(target_np)
        axes[0].set_title('Target Image')
        axes[0].axis('off')
        
        # 重构图像
        im2 = axes[1].imshow(best_recon_np)
        axes[1].set_title('Reconstruction')
        axes[1].axis('off')
    elif args.dimensions == 3:
        # 显示中间切片
        slice_idx = target_np.shape[0] // 2
        
        # 目标图像
        im1 = axes[0].imshow(target_np[slice_idx])
        axes[0].set_title(f'Target Image (Slice {slice_idx})')
        axes[0].axis('off')
        
        # 重构图像
        im2 = axes[1].imshow(best_recon_np[slice_idx])
...
'''def plot_average_loss(all_losses_list, args):
    """
    计算所有样本的平均loss并绘制曲线
    
    参数:
        all_losses_list: 包含所有样本训练过程中loss列表的列表
        args: 参数配置
    """
    import matplotlib.pyplot as plt
    import numpy as np
    ...
    # 找到所有loss列表中的最短长度，确保可以正确计算平均值
    min_length = min(len(losses) for losses in all_losses_list)
    
    # 截断所有loss列表到相同长度
    truncated_losses = [losses[:min_length] for losses in all_losses_list]
    
    # 计算每个迭代步骤的平均loss
    avg_losses = np.mean(truncated_losses, axis=0)
    
    # 创建保存目录
    save_dir = f'visualization/lossed/{args.data}_{args.end_reso}/{args.model}-{args.attack}'
    os.makedirs(save_dir, exist_ok=True)
    
    # 绘制平均loss曲线
    plt.figure(figsize=(10, 6))
    plt.plot(range(len(avg_losses)), avg_losses, label='Average Loss')
    plt.xlabel('Iterations')
    plt.ylabel('Loss')
    plt.title(f'Average Loss Across {len(all_losses_list)} Samples')
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.7)
    
    # 添加曲线下方的阴影来表示标准差
    if len(all_losses_list) > 1:
        std_losses = np.std(truncated_losses, axis=0)
        plt.fill_between(
            range(len(avg_losses)), 
    ...
    plt.figtext(0.5, 0.01, 
               f"Model: {args.model}, Rank: {args.max_rank}, Resolution: {args.end_reso}, Noise: {args.noise_std}", 
               ha='center', fontsize=9)
    
    # 保存图表
    save_path = f'{save_dir}/avg_loss_{args.model}_{args.max_rank}_{args.noise_std}_{time.strftime("%Y%m%d_%H%M%S")}.png'
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"平均loss曲线已保存至: {save_path}")
    
    return avg_losses'''

def plot_average_loss(all_losses_list, args):
    """
    绘制平均loss曲线，包括完整图及多个100步窗口的局部图，窗口起点根据diff_iterations累加生成

    参数:
        all_losses_list: 所有样本的loss列表
        args: 包含参数 diff_iterations, model, data 等
    """
    import matplotlib.pyplot as plt
    import numpy as np
    import os
    ...
    # 参数配置
    step_list = getattr(args, "iterations_for_upsampling", [])
    window_size = 100

    # 生成累加起始索引
    start_indices = [0]
    for step in step_list:
        start_indices.append(step)

    num_subplots = 1 + len(start_indices)  # 包括完整曲线的子图

    # 处理loss数据
    min_length = min(len(losses) for losses in all_losses_list)
    truncated_losses = [losses[:min_length] for losses in all_losses_list]
    avg_losses = np.mean(truncated_losses, axis=0)
    std_losses = np.std(truncated_losses, axis=0) if len(all_losses_list) > 1 else None
    ...
    # --- 子图 0: 全量 average loss ---
    axs[0].plot(range(len(avg_losses)), avg_losses, label='Average Loss')
    if std_losses is not None:
        axs[0].fill_between(range(len(avg_losses)), avg_losses - std_losses, avg_losses + std_losses, alpha=0.2, label='Std Dev')
    axs[0].set_title(f'Average Loss Across {len(all_losses_list)} Samples (Full)')
    axs[0].set_xlabel('Iterations')
    axs[0].set_ylabel('Loss')
    axs[0].grid(True, linestyle='--', alpha=0.7)
    axs[0].legend()

    # --- 子图 1..n: 局部窗口 average loss ---
    for i, start in enumerate(start_indices):
        ax = axs[i + 1]
        end = min(start + window_size, len(avg_losses))
        x_range = range(start, end)
        y_avg = avg_losses[start:end]
        y_std = std_losses[start:end] if std_losses is not None else None

        ax.plot(x_range, y_avg, label=f'Average Loss ({start}-{end})')
        if y_std is not None:
            ax.fill_between(x_range, y_avg - y_std, y_avg + y_std, alpha=0.2, label='Std Dev')
        ax.set_title(f'Average Loss (Iterations {start}-{end})')
    ...
    print(f"平均loss曲线（{num_subplots}子图，含自定义窗口）已保存至: {save_path}")
    return avg_losses


def load_all_adv_from_bin(bin_dir, max_num=None, use='x_adv'):
    """
    bin_dir : .bin 文件所在目录
    max_num : 只取前 max_num 个样本，例如 500；如果为 None 则全部加载
    返回:
        X_adv: (N, 3, H, W)
        Y:     (N,)
    """
    ...
    if max_num is not None:
        bin_files = bin_files[:max_num]   # 限制加载数量
    ...
    for f in bin_files:
        ...
        # ！！！取对抗样本 x_adv
        x_adv = pkg[use]      # shape = [1,3,224,224]
        y     = pkg["y"]          # shape = [1]

        X_list.append(x_adv)
        Y_list.append(y)

    X_adv = torch.cat(X_list, dim=0)  # [N,3,224,224]
